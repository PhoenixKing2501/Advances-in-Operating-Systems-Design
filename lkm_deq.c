...
const char *PROC_FILENAME = "partb_1_20CS30062_20CS30057"; // Replace with your roll number
...
static bool create_deque(struct proc_node *proc, int capacity)
{
...
static bool insert_proc_node(pid_t pid)
{
...
static int my_open(struct inode *inode, struct file *file)
{
    ...
    struct proc_node *cur = find_proc_node(pid);
    ...
    {
        pr_info("proc_open() invoked by Process pid %ld\n", pid);
        // check if pid already in list
        cur = find_proc_node(pid);
        if (cur != NULL)
        {
            pr_err("Process pid %ld already has this file open\n", pid);
            ret = -EACCES;
        }
        else
        {
            ...
            {
                pr_info("Process pid %ld successfully opened the file", pid);
                ret = 0;
            }
        }
    }
                        ...
                        size_t count, loff_t *position)
{
    ...
    {
        pr_info("proc_write() invoked by Process pid %ld\n", pid);
        // check if pid in list
        cur = find_proc_node(pid);
        if (cur == NULL)
        {
            pr_err("Process pid %ld has not opened this file\n");
            ret = -EACCES;
        }
        else
        {
...
static int helper_read(struct proc_node *cur)
{
    ...
    if (cur->state == PROC_FOPEN)
    {
        pr_err("Error:Deque not yet intialised for Process pid %ld\n", cur->pid);
        return -EACCES;
    }
    if (cur->deque->size == 0)
    {
    ...
    int data = cur->deque->front->data;
    // copy this data to ker_buf
    strncpy(ker_buf, &data, ker_buf_size);
    detele_from_front(cur->deque);
    return ker_buf_size;
}
                       ...
                       size_t count, loff_t *position)
{
    ...
    {
        pr_info("my_read() invoked by Process pid %ld\n", pid);
        cur = find_proc_node(pid);
        if (cur == NULL)
        {
            pr_err("Process pid %ld has not opened this file\n", pid);
        ...
        {
            ker_buf_size = min(count, MAX_READ_SIZE);
            ret = helper_read(cur);
        }
        print_deq(cur->deque);
        // debug printing of deque
...
static int my_release(struct inode *inode, struct file *file)
{
    ...
    {
        pr_info("my_release() invoked by Process pid %ld\n", pid);
        cur = find_proc_node(pid);
        if (cur == NULL)
        {
            pr_err("Process pid %ld has not opened this file\n", pid);
        ...
        else
        {
            ...
            pr_info("Process pid %ld successfully closed the file\n", pid);
            ret = 0;
        }
    }
    mutex_unlock(&file_mutex);
...
static void delete_process_list()
{
    struct proc_node *temp = proc_list;
    while (temp != NULL)
    {
